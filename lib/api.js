...
provide.factory('api', function(db) {
  ...
  api.get.punchcard = function(req, res) {
    getPunchcard().then(function(rows) {
      res.status(200).json(rows);
    }, error(res)).done();
  };
  
  var renderActivityGraph = require('./render-activity-graph.js');
  api.get.renderActivityGraph = function(req, res) {
    var width = req.query.width || 640,
    ...
    getPunchcard().then(function(data) {
      return renderActivityGraph(data, width, height);
    }).then(function(svg) {
      res.type('svg');
      res.status(200);
      res.send(svg);
