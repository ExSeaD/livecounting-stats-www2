...
provide.factory('api', function(db) {
  ...
  api.get.punchcard = function(req, res) {
    var q = join(
      ...
      'FROM (SELECT',
          'author,',
          'to_timestamp(created_utc/600*600) AS hour,',
          'COUNT(*) AS contribution_count',
        'FROM count_comments',
        'GROUP BY author, hour) AS t0',
      'RIGHT OUTER JOIN ((SELECT DISTINCT author FROM count_comments',
          'WHERE date_part(\'epoch\', CURRENT_TIMESTAMP - interval \'1 day\')::integer/600*600 <= created_utc LIMIT 7) AS t1',
        'CROSS JOIN generate_series(',
          'to_timestamp(date_part(\'epoch\', CURRENT_TIMESTAMP - interval \'1 day\')::integer/600*600),',
          'to_timestamp(date_part(\'epoch\', CURRENT_TIMESTAMP)::integer/600*600),',
          'interval \'600 seconds\') AS t2 (hour))',
      'USING (author, hour)',
      'GROUP BY author',
